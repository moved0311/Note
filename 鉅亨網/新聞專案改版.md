...
* add initial loading speed
```ts
import { QueryClientProvider, QueryClient } from 'react-query';
import { useEffect } from 'react';
import { getNews } from '../components/NewsList/NewsList';
...
useEffect(() => {
	const prefetchInifiteQuery = async () => {
	  await client.prefetchInfiniteQuery('news', getNews);
	};

	prefetchInifiteQuery();
	});
```
