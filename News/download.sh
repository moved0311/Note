...
root='/Users/taiyijiang/Desktop/Note/News'
now=$(date +"%Y-%m-%d")
...
for page in {1}
do
  curl "https://api.cnyes.com/media/api/v1/newslist/category/tw_stock?limit=100&page=${page}&from=${from}&to=${to}" -H "Accept: applcation/json;charset=UTF-8" | jq '.items.data' > $($root/data.json)
